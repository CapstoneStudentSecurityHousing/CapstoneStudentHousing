  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authError || !authData.user) {
        throw new Error('Authentication failed during verification. Please sign out and sign in again.');
      }
      const authUserId = authData.user.id;
      if (!authUserId) {
        throw new Error('Your user session is invalid. Please sign out and sign in again.');
      }
      ...
      console.error('Verification error details:', { message, status, details });

      if (message.toLowerCase().includes('row-level security') || message.toLowerCase().includes('policy')) {
        showNotification('Failed to submit verification due to policy restrictions. Make sure you are signed in and your profile is up to date.', 'error');
      } else if (message.toLowerCase().includes('lock broken') || details.toLowerCase().includes('steal')) {
        showNotification('Temporary database lock conflict. Please refresh the page and submit again.', 'error');
      } else {
