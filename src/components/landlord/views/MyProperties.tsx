          ...
          const syncInfo = async () => {
            const updates = missingInfo.map(p => 
              supabase.from('listings').update({
                ...
                landlord_phone: user.phone_number || '',
            ...
            <div className="relative aspect-[16/10] overflow-hidden">
              ...
              <div className="absolute top-4 right-4 flex gap-2">
                {property.is_verified && (
                  <span className="px-3 py-1 bg-emerald-500 text-white text-[10px] font-bold uppercase tracking-wider rounded-lg shadow-lg">
                    Verified
                  </span>
                )}
                <span className={`px-3 py-1 text-white text-[10px] font-bold uppercase tracking-wider rounded-lg shadow-lg ${
                  property.status === 'Active' ? 'bg-blue-500' : 
                  property.status === 'Rented' ? 'bg-slate-600' : 'bg-orange-500'
                }`}>
                  {property.status}
                </span>
              </div>
