...
import { User, Mail, Phone, MapPin, Shield, Bell, Lock, LogOut, Camera, ChevronRight, Save, X, Loader2 } from 'lucide-react';
...
export const Profile = ({ user }: { user: any }) => {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update all listings for this landlord to reflect profile changes
      const { error: listingsError } = await supabase
        .from('listings')
        .update({
          landlord_name: formData.fullName,
          landlord_phone: formData.phoneNumber,
      ...
      // Update all bookings for this landlord to reflect profile changes
        ...
        <div className="relative z-10 flex items-center gap-10">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              <h2 className="text-4xl font-black text-slate-900 tracking-tight">{user?.full_name || 'Landlord'}</h2>
              <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border ${
                user?.is_verified 
                  ? 'bg-emerald-50 text-emerald-600 border-emerald-100' 
                  : 'bg-slate-50 text-slate-400 border-slate-100'
              }`}>
                {user?.is_verified ? 'Verified' : 'Unverified'} Landlord
              </span>
            </div>
            <p className="text-slate-500 mb-6 font-medium">Property Owner • Member since {user?.created_at ? new Date(user.created_at).getFullYear() : '2025'}</p>
            ...
            <div className="flex items-center gap-8">
              ...
              <div className="flex items-center gap-2 text-slate-400">
                ...
                <span className="text-sm font-bold">{user?.phone_number || 'No phone set'}</span>
              </div>
            </div>
          </div>
          ...
          >
            <form onSubmit={handleUpdate} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-4">Tax ID / Registration Number</label>
                  <input
                    type="text"
            ...
            <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm space-y-6">
              ...
              <div className="space-y-4">
                {[
                  ...
                  { label: 'Personal Address', value: user?.address || 'Not set' },
                  { label: 'Business Name', value: user?.business_name || 'Not set' },
                  { label: 'Tax ID', value: user?.tax_id || 'Not set' },
