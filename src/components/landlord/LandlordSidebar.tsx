...
export const LandlordSidebar = ({ activeTab, setActiveTab, onLogout, user, isOpen = true, onClose }: SidebarProps) => {
  ...
  const menuItems = [
    ...
    { id: 'disputes', label: 'Disputes', icon: <AlertCircle className="w-5 h-5" />, badge: counts.disputes },
    { id: 'compliance', label: 'Compliance', icon: <ShieldCheck className="w-5 h-5" /> },
    { id: 'profile', label: 'My Profile', icon: <LayoutDashboard className="w-5 h-5" /> },
  ];

  return (
    <aside className={`bg-[#0F172A] border-r border-slate-800 flex flex-col fixed h-screen z-50 transition-all ${isOpen ? 'left-0' : '-left-full'} md:left-0 md:w-72 md:relative md:h-auto md:z-10`}>
      <div className="md:hidden p-4 flex items-center justify-between border-b border-slate-800">
        <div className="flex items-center gap-2">
          ...
          <div>
            <span className="text-xl font-bold text-white tracking-tight">SafeStay</span>
          </div>
        </div>
        {onClose && (
