...
export const AuditLogs: React.FC = () => {
  ...
  const filteredLogs = logs.filter(log => {
    const safeText = (val?: string) => (val ?? '').toString().toLowerCase();
    const ts = searchTerm.toLowerCase();

    const matchesSearch = 
      safeText(log.details).includes(ts) ||
      safeText(log.action).includes(ts) ||
      safeText(log.adminName).includes(ts) ||
      safeText(log.targetId).includes(ts);
